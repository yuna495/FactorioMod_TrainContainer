local bounding_box = require('scripts.bounding_box')
...
--- @param entities LuaEntity[]
--- @param target_entity_name string
--- @param is_ghost boolean
--- @return ChestGroup[]
local function group_chests(entities, target_entity_name, is_ghost)
	local chest_grid = MergingChests.entities_to_grid(entities)
	local width = chest_grid.max_x - chest_grid.min_x + 1
	local height = chest_grid.max_y - chest_grid.min_y + 1
	...
	if #entities ~= width * height then
		return {}
	end

	for x = chest_grid.min_x, chest_grid.max_x do
		for y = chest_grid.min_y, chest_grid.max_y do
			if not chest_grid[x] or not chest_grid[x][y] then
				return {}
			end
		end
	end

	local merged_chest_name = MergingChests.get_merged_chest_name(target_entity_name, width, height)
	if prototypes.entity[merged_chest_name] == nil then
		return {}
	end
	...
	return {
		{
			entities = entities,
			merged_chest_name = merged_chest_name,
			bounding_box = {
				left_top = {
					x = chest_grid.min_x,
					y = chest_grid.min_y
				...
				right_bottom = {
					x = chest_grid.max_x + 1,
					y = chest_grid.max_y + 1
				}
			},
			is_ghost = is_ghost,
		}
...
local function create_merged_chest(player, chest_name, position, is_ghost, bar, quality)
	...
	if prototypes.entity[chest_name].type ~= 'infinity-container' then
		entity_data.bar = math.min(bar, 65535)
	end
	if is_ghost then
