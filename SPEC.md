...
Schema version 3 stores `storage.train_transfer.filters[unit_number] = { mode = "whitelist" | "blacklist", slots = { [1..5] = { name = ITEM, quality = QUALITY } } }`, with absent indices for empty slots. An absent configuration means an empty whitelist. The configuration is independent of loading mode, remains editable while loading is off, is discarded on entity removal/split, and is not saved to blueprints or copied by settings copy/paste. Old single item-and-quality filters migrate to whitelist slot 1; older string-only filters migrate to whitelist slot 1 with normal quality. Unknown item prototypes are discarded and unavailable qualities fall back to normal, as before. Migrating also updates saved active-group filter snapshots and clears their retry deadline so legacy settings are not retained there.
